...
import {quarterlyIncomeStatements} from "../../services/api";
...
const QuarterlyIncomeStatement: React.FC<IncomeStatementProps> = ({symbol}) => {
    const [incomeStatements, setIncomeStatements] = React.useState<IncomeStatement[]>();
    React.useEffect(() => {
        quarterlyIncomeStatements(symbol).then((data) => {
...
export default QuarterlyIncomeStatement;